const uiViewLogic = (() => {
    ...
    function createGueteContentHTML(stats, methode, kollektivName) {
        ...
        const getTooltip = (key) => (TOOLTIP_CONTENT.statMetrics[key]?.description || key).replace(/\[METHODE\]/g, methode);
        const getTooltipInterp = (key, metricData) => { const digits = (key === 'f1' || key === 'auc') ? 3 : 1; const isPercent = !(key === 'f1' || key === 'auc'); return (TOOLTIP_CONTENT.statMetrics[key]?.interpretation || '').replace(/\[METHODE\]/g, methode).replace(/\[WERT\]/g, formatNumber(metricData?.value, digits, na)).replace(/\[LOWER\]/g, formatNumber(metricData?.ci?.lower, digits, na)).replace(/\[UPPER\]/g, formatNumber(metricData?.ci?.upper, digits, na)).replace(/\[KOLLEKTIV\]/g, kollektivName).replace(/\[METHOD_CI\]/g, metricData?.method || na).replace(/\[BEWERTUNG\]/g, key === 'auc' ? getAUCBewertung(metricData?.value) : '').replace(/%/g, '%'); };
        ...
        let metricsHTML = `<div class="table-responsive px-2"><table class="table table-sm table-striped small mb-0 caption-top" id="table-guete-metrics-${methode}-${kollektivName.replace(/\s+/g, '_')}"><caption>Diagnostische Gütekriterien</caption><thead><tr><th>Metrik</th><th>Wert (95% CI)</th><th>CI Methode</th></tr></thead><tbody>
            <tr><td data-tippy-content="${getTooltip('sens')}">Sensitivität</td><td data-tippy-content="${getTooltipInterp('sens', stats.sens)}">${fCI_perf(stats.sens, 'sens')}</td><td>${stats.sens?.method || na}</td></tr>
            <tr><td data-tippy-content="${getTooltip('spez')}">Spezifität</td><td data-tippy-content="${getTooltipInterp('spez', stats.spez)}">${fCI_perf(stats.spez, 'spez')}</td><td>${stats.spez?.method || na}</td></tr>
            <tr><td data-tippy-content="${getTooltip('ppv')}">PPV</td><td data-tippy-content="${getTooltipInterp('ppv', stats.ppv)}">${fCI_perf(stats.ppv, 'ppv')}</td><td>${stats.ppv?.method || na}</td></tr>
            <tr><td data-tippy-content="${getTooltip('npv')}">NPV</td><td data-tippy-content="${getTooltipInterp('npv', stats.npv)}">${fCI_perf(stats.npv, 'npv')}</td><td>${stats.npv?.method || na}</td></tr>
            <tr><td data-tippy-content="${getTooltip('acc')}">Accuracy</td><td data-tippy-content="${getTooltipInterp('acc', stats.acc)}">${fCI_perf(stats.acc, 'acc')}</td><td>${stats.acc?.method || na}</td></tr>
            <tr><td data-tippy-content="${getTooltip('balAcc')}">Balanced Accuracy</td><td data-tippy-content="${getTooltipInterp('balAcc', stats.balAcc)}">${fCI_perf(stats.balAcc, 'balAcc')}</td><td>${stats.balAcc?.method || na}</td></tr>
            <tr><td data-tippy-content="${getTooltip('f1')}">F1-Score</td><td data-tippy-content="${getTooltipInterp('f1', stats.f1)}">${fCI_perf(stats.f1, 'f1')}</td><td>${stats.f1?.method || na}</td></tr>
            <tr><td data-tippy-content="${getTooltip('auc')}">AUC (Bal. Acc.)</td><td data-tippy-content="${getTooltipInterp('auc', stats.auc)}">${fCI_perf(stats.auc, 'auc')}</td><td>${stats.auc?.method || na}</td></tr>
        </tbody></table></div>`;
    ...
    function createVergleichContentHTML(stats, kollektivName) {
        ...
        const mcNemarTooltip = TOOLTIP_CONTENT.statMetrics.mcnemar.description; const deLongTooltip = TOOLTIP_CONTENT.statMetrics.delong.description;
        const getPValueTooltip = (pVal, testKey) => (TOOLTIP_CONTENT.statMetrics[testKey]?.interpretation || TOOLTIP_CONTENT.statMetrics.defaultP.interpretation).replace('[P_WERT]', fP(pVal)).replace('[SIGNIFIKANZ]', getStatisticalSignificanceSymbol(pVal)).replace('[KOLLEKTIV]', kollektivName).replace('[SIGNIFIKANZ_TEXT]', getStatisticalSignificanceText(pVal));
        let tableHTML = `<div class="table-responsive px-2"><table class="table table-sm table-striped small mb-0" id="table-vergleich-as-vs-t2-${kollektivName.replace(/\s+/g, '_')}"><thead><tr><th>Test</th><th>Statistik</th><th>p-Wert</th><th>Methode</th></tr></thead><tbody>
            <tr><td data-tippy-content="${mcNemarTooltip}">McNemar (Accuracy)</td><td>${formatNumber(stats.mcnemar?.statistic, 3, na)} (df=${stats.mcnemar?.df || na})</td><td data-tippy-content="${getPValueTooltip(stats.mcnemar?.pValue, 'mcnemar')}">${fP(stats.mcnemar?.pValue)} ${getStatisticalSignificanceSymbol(stats.mcnemar?.pValue)}</td><td>${stats.mcnemar?.method || na}</td></tr>
            <tr><td data-tippy-content="${deLongTooltip}">DeLong (AUC)</td><td>Z=${formatNumber(stats.delong?.Z, 3, na)}</td><td data-tippy-content="${getPValueTooltip(stats.delong?.pValue, 'delong')}">${fP(stats.delong?.pValue)} ${getStatisticalSignificanceSymbol(stats.delong?.pValue)}</td><td>${stats.delong?.method || na}</td></tr>
        </tbody></table></div>`;
        return tableHTML;
    }
    ...
    function createAssoziationContentHTML(stats, kollektivName, criteria) {
        ...
        const addRow = (name, key, assocObj, isActive = true) => { if (!assocObj) return ''; const orStr = formatCI(assocObj.or?.value, assocObj.or?.ci?.lower, assocObj.or?.ci?.upper, 2, false, na); const rdValPerc = formatNumber(assocObj.rd?.value !== null && !isNaN(assocObj.rd?.value) ? assocObj.rd.value * 100 : NaN, 1, na); const rdCILowerPerc = formatNumber(assocObj.rd?.ci?.lower !== null && !isNaN(assocObj.rd?.ci?.lower) ? assocObj.rd.ci.lower * 100 : NaN, 1, na); const rdCIUpperPerc = formatNumber(assocObj.rd?.ci?.upper !== null && !isNaN(assocObj.rd?.ci?.upper) ? assocObj.rd.ci.upper * 100 : NaN, 1, na); const rdStr = `${rdValPerc} (${rdCILowerPerc} - ${rdCIUpperPerc})`; const phiStr = formatNumber(assocObj.phi?.value, 2, na); const pStr = fP(assocObj.pValue); const sigSymbol = getStatisticalSignificanceSymbol(assocObj.pValue); const sigText = getStatisticalSignificanceText(assocObj.pValue); const testName = assocObj.testName || na; const pTooltipKey = testName.includes("Fisher") ? 'fisher' : (testName.includes("Mann-Whitney") ? 'mannwhitney' : 'defaultP'); const merkmalName = assocObj.featureName || name; const aktivText = isActive ? '' : ' <small class="text-muted">(inaktiv)</small>'; const orTooltip = (TOOLTIP_CONTENT.statMetrics.or.interpretation || '').replace('[WERT]', formatNumber(assocObj.or?.value, 2, na)).replace('[FAKTOR_TEXT]', assocObj.or?.value > 1 ? TOOLTIP_CONTENT.statMetrics.orFaktorTexte.ERHOEHT : (assocObj.or?.value < 1 ? TOOLTIP_CONTENT.statMetrics.orFaktorTexte.VERRINGERT : 'unverändert')).replace('[LOWER]', formatNumber(assocObj.or?.ci?.lower, 2, na)).replace('[UPPER]', formatNumber(assocObj.or?.ci?.upper, 2, na)).replace('[P_WERT]', pStr).replace('[SIGNIFIKANZ]', sigSymbol).replace('[MERKMAL]', merkmalName).replace('[KOLLEKTIV]', kollektivName).replace('[METHOD_CI]', assocObj.or?.method || na); const rdTooltip = (TOOLTIP_CONTENT.statMetrics.rd.interpretation || '').replace('[WERT]', rdValPerc).replace('[HOEHER_NIEDRIGER]', assocObj.rd?.value > 0 ? TOOLTIP_CONTENT.statMetrics.rdRichtungTexte.HOEHER : (assocObj.rd?.value < 0 ? TOOLTIP_CONTENT.statMetrics.rdRichtungTexte.NIEDRIGER : 'gleich')).replace('[LOWER]', rdCILowerPerc).replace('[UPPER]', rdCIUpperPerc).replace('[MERKMAL]', merkmalName).replace('[KOLLEKTIV]', kollektivName).replace('[METHOD_CI]', assocObj.rd?.method || na); const phiTooltip = (TOOLTIP_CONTENT.statMetrics.phi.interpretation || '').replace('[WERT]', phiStr).replace('[STAERKE]', getPhiBewertung(assocObj.phi?.value)).replace('[MERKMAL]', merkmalName).replace('[KOLLEKTIV]', kollektivName); const pTooltip = (TOOLTIP_CONTENT.statMetrics[pTooltipKey]?.interpretation || TOOLTIP_CONTENT.statMetrics.defaultP.interpretation).replace('[P_WERT]', pStr).replace('[SIGNIFIKANZ]', sigSymbol).replace('[MERKMAL]', merkmalName).replace('[VARIABLE]', merkmalName).replace('[SIGNIFIKANZ_TEXT]', sigText).replace('[KOLLEKTIV]', kollektivName); return `<tr><td>${merkmalName}${aktivText}</td><td data-tippy-content="${orTooltip}">${orStr}</td><td data-tippy-content="${rdTooltip}">${rdStr}</td><td data-tippy-content="${phiTooltip}">${phiStr}</td><td data-tippy-content="${pTooltip}">${pStr} ${sigSymbol}</td><td data-tippy-content="${(TOOLTIP_CONTENT.statMetrics[pTooltipKey]?.description || '').replace('[MERKMAL]', merkmalName).replace('[VARIABLE]', merkmalName)}">${testName}</td></tr>`; };
        if (stats.as) tableHTML += addRow('AS Positiv', 'as', stats.as);
        if (stats.size_mwu && stats.size_mwu.testName && !stats.size_mwu.testName.includes("Invalid") && !stats.size_mwu.testName.includes("Nicht genug")) { const mwuObj = stats.size_mwu; const pStr = fP(mwuObj.pValue); const sigSymbol = getStatisticalSignificanceSymbol(mwuObj.pValue); const sigText = getStatisticalSignificanceText(mwuObj.pValue); const pTooltip = (TOOLTIP_CONTENT.statMetrics.mannwhitney?.interpretation || '').replace('[VARIABLE]', 'LK Größe').replace('[P_WERT]', pStr).replace('[SIGNIFIKANZ]', sigSymbol).replace('[SIGNIFIKANZ_TEXT]', sigText).replace('[KOLLEKTIV]', kollektivName); tableHTML += `<tr><td>LK Größe (Median Vgl.)</td><td>${na}</td><td>${na}</td><td>${na}</td><td data-tippy-content="${pTooltip}">${pStr} ${sigSymbol}</td><td data-tippy-content="${(TOOLTIP_CONTENT.statMetrics.mannwhitney?.description || '').replace('[VARIABLE]','LK Größe')}">${mwuObj.testName || na}</td></tr>`; }
        const featureOrder = ['size', 'form', 'kontur', 'homogenitaet', 'signal']; featureOrder.forEach(key => { if (stats[key]) { const isActive = criteria[key]?.active === true; tableHTML += addRow(stats[key].featureName || `T2: ${key}`, key, stats[key], isActive); } });
    ...
    function createVergleichKollektiveContentHTML(stats, kollektiv1Name, kollektiv2Name) {
        ...
        const getPValueTooltip = (pValue, testKey, methode) => (TOOLTIP_CONTENT.statMetrics[testKey]?.interpretation || '').replace('[METHODE]', methode).replace('[KOLLEKTIV1]', kollektiv1Display).replace('[KOLLEKTIV2]', kollektiv2Display).replace('[SIGNIFIKANZ_TEXT]', getStatisticalSignificanceText(pValue)).replace('[P_WERT]', fP(pValue));
        ...
        tableHTML += `<tr><td>Accuracy</td><td>AS</td><td data-tippy-content="${getPValueTooltip(accAS?.pValue, 'accComp', 'AS')}">${fP(accAS?.pValue)} ${getStatisticalSignificanceSymbol(accAS?.pValue)}</td><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.accComp?.description.replace('[METHODE]','AS')}">${accAS?.testName || na}</td></tr>`;
        tableHTML += `<tr><td>Accuracy</td><td>T2</td><td data-tippy-content="${getPValueTooltip(accT2?.pValue, 'accComp', 'T2')}">${fP(accT2?.pValue)} ${getStatisticalSignificanceSymbol(accT2?.pValue)}</td><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.accComp?.description.replace('[METHODE]','T2')}">${accT2?.testName || na}</td></tr>`;
    ...
    function createCriteriaComparisonTableHTML(results, kollektivName) {
         ...
         const tc = TOOLTIP_CONTENT || {}; const cc = tc.criteriaComparison || {}; const sm = tc.statMetrics || {}; const fallbackDesc = (k, m) => (sm[k]?.description || k).replace('[METHODE]', m);
         const headers = [cc.tableHeaderSet || "Methode / Kriteriensatz", cc.tableHeaderSens || "Sens.", cc.tableHeaderSpez || "Spez.", cc.tableHeaderPPV || "PPV", cc.tableHeaderNPV || "NPV", cc.tableHeaderAcc || "Acc.", cc.tableHeaderAUC || "AUC/BalAcc"]; const tableId = "table-kriterien-vergleich";
         let tableHTML = `<div class="table-responsive px-2"><table class="table table-sm table-striped table-hover small caption-top" id="${tableId}"><caption>Vergleich verschiedener Kriteriensätze (vs. N) für Kollektiv: ${kollektivName}</caption><thead class="small"><tr><th>${headers[0]}</th><th data-tippy-content="${fallbackDesc('sens', 'die Methode')}">${headers[1]}</th><th data-tippy-content="${fallbackDesc('spez', 'die Methode')}">${headers[2]}</th><th data-tippy-content="${fallbackDesc('ppv', 'die Methode')}">${headers[3]}</th><th data-tippy-content="${fallbackDesc('npv', 'die Methode')}">${headers[4]}</th><th data-tippy-content="${fallbackDesc('acc', 'die Methode')}">${headers[5]}</th><th data-tippy-content="${fallbackDesc('auc', 'die Methode')}">${headers[6]}</th></tr></thead><tbody>`;
         results.forEach(result => {
             ...
             const getTooltip = (key) => { const interpretation = sm[key]?.interpretation || fallbackDesc(key, nameDisplay); const value = result[key]; const digits = (key === 'auc') ? 3 : 1; const isPercent = !(key === 'auc'); return interpretation.replace(/\[METHODE\]/g, nameDisplay).replace(/\[WERT\]/g, isPercent ? formatPercent(value, digits, '--') : formatNumber(value, digits, '--')).replace(/\(95% CI nach \[METHOD_CI\]: \[LOWER\] - \[UPPER\]\)/g, '(Kein CI in Übersicht)').replace(/\[KOLLEKTIV\]/g, kollektivName).replace(/\[BEWERTUNG\]/g, key === 'auc' ? getAUCBewertung(result.auc) : '').replace(/%/g, '%'); };
             tableHTML += `<tr class="${rowClass}"><td class="fw-bold">${nameDisplay}</td><td data-tippy-content="${getTooltip('sens')}">${formatPercent(result.sens, 1)}</td><td data-tippy-content="${getTooltip('spez')}">${formatPercent(result.spez, 1)}</td><td data-tippy-content="${getTooltip('ppv')}">${formatPercent(result.ppv, 1)}</td><td data-tippy-content="${getTooltip('npv')}">${formatPercent(result.npv, 1)}</td><td data-tippy-content="${getTooltip('acc')}">${formatPercent(result.acc, 1)}</td><td data-tippy-content="${getTooltip('auc')}">${formatNumber(result.auc, 3)}</td></tr>`;
         });
         tableHTML += `</tbody></table></div>`; return tableHTML;
    }
    ...
    function _createPresentationView_ASPUR_HTML(presentationData) {
        ...
        const createPerfTableRow = (stats, kollektivKey) => { const kollektivDisplayName = getKollektivDisplayName(kollektivKey); if (!stats || typeof stats.matrix !== 'object') return `<tr><td class="fw-bold">${kollektivDisplayName} (N=?)</td><td colspan="6" class="text-muted text-center">Daten fehlen</td></tr>`; const na = '--'; const fCI_p = (m, k) => { const d = (k === 'auc'||k==='f1') ? 3 : 1; const p = !(k === 'auc'||k==='f1'); return formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, d, p, na); }; const count = stats.matrix ? (stats.matrix.rp + stats.matrix.fp + stats.matrix.fn + stats.matrix.rn) : 0; return `<tr><td class="fw-bold">${kollektivDisplayName} (N=${count})</td><td>${fCI_p(stats.sens, 'sens')}</td><td>${fCI_p(stats.spez, 'spez')}</td><td>${fCI_p(stats.ppv, 'ppv')}</td><td>${fCI_p(stats.npv, 'npv')}</td><td>${fCI_p(stats.acc, 'acc')}</td><td>${fCI_p(stats.auc, 'auc')}</td></tr>`; };
    ...
    function _createPresentationView_ASvsT2_HTML(presentationData, selectedStudyId = null, currentKollektiv = 'Gesamt') {
        ...
        if (canDisplayResults) {
            const fPVal = (r,d=3) => { const p = r?.pValue; return (p !== null && !isNaN(p)) ? (p < 0.001 ? '<0.001' : formatNumber(p, d)) : '--'; };
            ...
            const compTitle = `Stat. Vergleich (AS vs. ${t2CriteriaLabelShort || 'T2'})`; const perfTitle = `Vergleich Metriken (AS vs. ${t2CriteriaLabelShort || 'T2'})`; const chartTitle = `Vergleichs-Chart (AS vs. ${t2CriteriaLabelShort || 'T2'})`;
            ...
            let comparisonTableHTML = `<div class="table-responsive"><table class="table table-sm table-striped small mb-0" id="${perfTableId}"><thead class="small"><tr><th>Metrik</th><th>AS (Wert, 95% CI)</th><th>${t2CriteriaLabelShort || 'T2'} (Wert, 95% CI)</th></tr></thead><tbody>`;
            ...
            testsTableHTML += `<tr><td>McNemar (Acc)</td><td>${formatNumber(vergleich?.mcnemar?.statistic, 3, '--')} (df=${vergleich?.mcnemar?.df || '--'})</td><td> ${fPVal(vergleich?.mcnemar)} ${getStatisticalSignificanceSymbol(vergleich?.mcnemar?.pValue)}</td><td class="text-muted">${vergleich?.mcnemar?.method || '--'}</td></tr>`;
            testsTableHTML += `<tr><td>DeLong (AUC)</td><td>Z=${formatNumber(vergleich?.delong?.Z, 3, '--')}</td><td> ${fPVal(vergleich?.delong)} ${getStatisticalSignificanceSymbol(vergleich?.delong?.pValue)}</td><td class="text-muted">${vergleich?.delong?.method || '--'}</td></tr>`;
                     ...
                     <div class="col-lg-7 col-xl-7 presentation-comparison-col-left">
                        <div class="card h-100">
                            ...
                            <div class="card-body p-1 d-flex align-items-center justify-content-center">
                                 <div id="${chartContainerId}" class="praes-chart-container w-100" style="min-height: 300px;" data-tippy-content="Balkendiagramm: Vergleich der Gütekriterien (AS vs. ${t2CriteriaLabelShort || 'T2'}).">
