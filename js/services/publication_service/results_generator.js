window.resultsGenerator = (() => {
    ...
    function generatePatientCharacteristicsHTML(stats, commonData) {
        ...
        const menNeoadjuvantPercent = helpers.formatMetricForPublication({value: (neoadjuvantStats?.descriptive?.sex?.m || 0) / nNeoadjuvantTherapy, n_success: (neoadjuvantStats?.descriptive?.sex?.m || 0), n_trials: nNeoadjuvantTherapy}, 'acc', { includeCI: false, includeCount: true });
        const menSurgeryPercent = helpers.formatMetricForPublication({value: (surgeryAloneStats?.descriptive?.sex?.m || 0) / nSurgeryAlone, n_success: (surgeryAloneStats?.descriptive?.sex?.m || 0), n_trials: nSurgeryAlone}, 'acc', { includeCI: false, includeCount: true });

        const text = `
            <h3 id="ergebnisse_patientencharakteristika">Patient Characteristics</h3>
            <p>The study cohort comprised ${helpers.formatValueForPublication(nOverall, 0)} patients (mean age, ${helpers.formatValueForPublication(overallStats?.descriptive?.age?.mean, 1)} years ± ${helpers.formatValueForPublication(overallStats?.descriptive?.age?.sd, 1)} [standard deviation]; ${overallStats?.descriptive?.sex?.m} men). The process of patient enrollment is detailed in the study flowchart (Fig 1). Of the included patients, ${surgeryAlonePercentString} underwent primary surgery, and ${neoadjuvantPercentString} received neoadjuvant chemoradiotherapy. Overall, ${nPositivePercentString} had histopathologically confirmed lymph node metastases (N-positive). Patients in the primary surgery group were significantly older than those in the neoadjuvant therapy group (mean age, ${helpers.formatValueForPublication(surgeryAloneStats?.descriptive?.age?.mean, 1)} vs ${helpers.formatValueForPublication(neoadjuvantStats?.descriptive?.age?.mean, 1)} years; ${helpers.formatPValueForPublication(descriptiveComparison?.age?.pValue)}), and the proportion of men was significantly higher in the neoadjuvant group (${menNeoadjuvantPercent} vs ${menSurgeryPercent}; ${helpers.formatPValueForPublication(descriptiveComparison?.sex?.pValue)}). There was no significant difference in the prevalence of N-positive status between the subgroups (${helpers.formatPValueForPublication(descriptiveComparison?.nStatus?.pValue)}). Detailed patient characteristics for the overall cohort and by treatment subgroup are provided in Table 1.</p>
        `;
            ...
            <div class="my-4 p-3 border rounded bg-light" id="figure-2-examples-container-wrapper">
                ...
                <div id="figure-2-examples-container" class="publication-chart-container text-start small">
                    ...
                    <ul class="list-unstyled ps-3">
                        ...
                        <li><strong>Panel D:</strong> Corresponding contrast-enhanced T1-weighted VIBE image demonstrating a positive Avocado Sign in this otherwise unremarkable node, highlighting the added value of the sign.</li>
                    </ul>
                    <hr>
        ...
        const getCountRow = (count, total) => {
            ...
            return helpers.formatMetricForPublication({ value: count / total, n_success: count, n_trials: total }, 'acc', { includeCI: false, includeCount: true });
        };
