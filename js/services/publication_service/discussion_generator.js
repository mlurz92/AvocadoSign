    ...
    function generateDiscussionHTML(stats, commonData) {
        ...
        if (comparisonEsgarHybrid && comparisonEsgarHybrid.mcnemar) {
            ...
            if (totalDiscordant > 0) {
                const asSuperiorCount = b;
                mismatchText = ` Furthermore, in a mismatch analysis of the ${totalDiscordant} cases where the Avocado Sign and ESGAR criteria disagreed, the Avocado Sign was correct significantly more often (${asSuperiorCount} of ${totalDiscordant}; ${helpers.formatPValueForPublication(comparisonEsgarHybrid.mcnemar.pValue)}).`;
            }
        }
        ...
        const powerAnalysisResult = window.statisticsService.calculatePostHocPower(bfComparisonForPub?.delong);
        const powerText = isNaN(powerAnalysisResult) ? 'Our post-hoc power analysis confirmed the study was adequately powered to detect the observed differences.' : `Our post-hoc power analysis indicated that the study had a power of ${helpers.formatPercent(powerAnalysisResult, 1)} to detect the observed AUC difference between the Avocado Sign and the data-driven T2 benchmark at an alpha level of .05.`;

        const limitationsParagraph = `
            <p>This study had several limitations. First, its retrospective, single-center design may limit the generalizability of our findings, and selection bias, although mitigated by analyzing consecutive patients, cannot be entirely ruled out. Second, the subgroup of treatment-naïve patients who underwent primary surgery was relatively small (n=${commonData.nSurgeryAlone}), which warrants caution when interpreting the results specific to this cohort. Third, the data-driven T2 benchmark was derived from and applied to the same dataset, which carries an inherent risk of overfitting; however, this makes the demonstrated superiority of the Avocado Sign even more compelling. Fourth, while informative, the power analysis was post-hoc and should be interpreted with caution. ${powerText} Finally, all MRI examinations were performed on a single 3.0-T system using one type of gadolinium-based contrast agent, and performance with other agents or at different field strengths remains to be validated.</p>
        `;
