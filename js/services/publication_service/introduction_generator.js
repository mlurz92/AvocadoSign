window.introductionGenerator = (() => {
    ...
    function generateIntroductionHTML(stats, commonData) {
        ...
        const introText = `
            ...
            <p>This limitation stems from the reliance on T2-weighted (T2w) criteria, including node size, border contour, and internal signal characteristics, which have demonstrated inconsistent and often suboptimal diagnostic accuracy for predicting the patient-level nodal status. A pivotal meta-analysis by Al-Sukhni et al reported a pooled sensitivity and specificity of only 77% and 71%, respectively, for T2w-based MRI ${helpers.getReference('Al_Sukhni_2012')}. This diagnostic uncertainty is further highlighted by the large prospective OCUM trial, which reported an accuracy of only 56.5% for T2w-based nodal staging and consequently favored treatment decisions based on the mesorectal fascia status due to the low reliability of N-staging ${helpers.getReference('Stelzner_2022')}. This diagnostic gap can lead to both over- and undertreatment, highlighting a critical need for more robust and reproducible imaging markers.</p>
