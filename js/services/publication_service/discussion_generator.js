window.discussionGenerator = (() => {
    ...
    function generateDiscussionHTML(stats, commonData) {
        ...
        const summaryParagraph = `
            <p>In this study, our primary objective was to rigorously evaluate the diagnostic utility of the contrast-enhanced Avocado Sign by comparing it first against a computationally optimized, data-driven T2-based benchmark. Our central finding is that the Avocado Sign, a simple binary imaging marker, provides high diagnostic accuracy (AUC, ${helpers.formatMetricForPublication(performanceAS?.auc, 'auc', { includeCI: false, includeCount: false })}) that was statistically comparable with this 'best-case' T2 benchmark ${bfComparisonText}. This suggests that the Avocado Sign efficiently captures a high degree of the diagnostically relevant information available in a full MRI examination, but does so through a simple, binary assessment that is less prone to the complexities and potential overfitting of multiparametric T2 models.</p>
        `;

        const contextParagraph = `
            <p>To contextualize these findings, we then compared the Avocado Sign to established, literature-based T2 criteria. The limitations of these conventional criteria are well-documented, with meta-analyses reporting suboptimal accuracy ${helpers.getReference('Al_Sukhni_2012')}${helpers.getReference('Zhuang_2021')}. Our results align with this, showing the Avocado Sign to be superior to guidelines such as the ESGAR 2016 consensus criteria ${helpers.getReference('Rutegard_2025')} and simpler size-based restaging rules ${helpers.getReference('Pangarkar_2021')}${helpers.getReference('Zhang_2023')}. This diagnostic gap in T2-staging is a key reason why the OCUM trial favored treatment decisions based on mesorectal fascia status ${helpers.getReference('Stelzner_2022')}. We postulate that the Avocado Sign's superior performance may stem from its ability to detect altered vascularity and central necrosis, features that may identify not only true nodal metastases but also prognostically significant tumor deposits, which are often indistinguishable on T2w images ${helpers.getReference('Lord_2019')}. This hypothesis is supported by the sign's almost perfect interobserver agreement (Cohen’s kappa = ${helpers.formatValueForPublication(overallStats?.interobserverKappa?.value, 2, false, true)}), which indicates it is a simple and highly reproducible marker ${helpers.getReference('Lurz_Schaefer_2025')}. ${interCohortComparisonText}</p>
        `;
