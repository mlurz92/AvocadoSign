const uiViewLogic = (() => {
    ...
    function _getStatTooltipHTML(metricKey, statsObject, methodName, kollektivName, t2ShortName = null) {
        const metricInfo = TOOLTIP_CONTENT.statMetrics[metricKey];
        if (!metricInfo) return `Unbekannte Metrik: ${metricKey}`;

        const description = (metricInfo.description || metricKey).replace(/\[METHODE\]/g, methodName);
        let interpretation = metricInfo.interpretation || 'Keine Interpretation verfügbar.';

        const metricData = statsObject ? statsObject[metricKey] : null;
        const na = '--';
        ...
        const bewertungStr = (metricKey === 'auc') ? getAUCBewertung(metricData?.value) : '';

        interpretation = interpretation
            .replace(/\[METHODE\]/g, methodName)
            ...
            .replace(/\[T2_SHORT_NAME\]/g, t2ShortName || 'T2');

        return `${description}<hr class='my-1'>${interpretation}`;
    }

    function _getTestTooltipHTML(testKey, statsObject, kollektivName, t2ShortName = null) {
         const testInfo = TOOLTIP_CONTENT.statMetrics[testKey];
         if (!testInfo) return `Unbekannter Test: ${testKey}`;

         const description = (testInfo.description || testKey);
         let interpretation = testInfo.interpretation || 'Keine Interpretation verfügbar.';
         const na = '--';
         const testData = statsObject ? statsObject[testKey] : null;
    ...
    function createGueteContentHTML(stats, methode, kollektivName) {
        ...
        let metricsHTML = `<div class="table-responsive px-2"><table class="table table-sm table-striped small mb-0 caption-top" id="table-guete-metrics-${methode}-${kollektivName.replace(/\s+/g, '_')}"><caption>Diagnostische Gütekriterien</caption><thead><tr><th>Metrik</th><th>Wert (95% CI)</th><th>CI Methode</th></tr></thead><tbody>
            <tr><td data-tippy-content="${_getStatTooltipHTML('sens', null, methode, kollektivName)}">Sensitivität</td><td data-tippy-content="${_getStatTooltipHTML('sens', stats, methode, kollektivName)}">${fCI_perf(stats.sens, 'sens')}</td><td>${stats.sens?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getStatTooltipHTML('spez', null, methode, kollektivName)}">Spezifität</td><td data-tippy-content="${_getStatTooltipHTML('spez', stats, methode, kollektivName)}">${fCI_perf(stats.spez, 'spez')}</td><td>${stats.spez?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getStatTooltipHTML('ppv', null, methode, kollektivName)}">PPV</td><td data-tippy-content="${_getStatTooltipHTML('ppv', stats, methode, kollektivName)}">${fCI_perf(stats.ppv, 'ppv')}</td><td>${stats.ppv?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getStatTooltipHTML('npv', null, methode, kollektivName)}">NPV</td><td data-tippy-content="${_getStatTooltipHTML('npv', stats, methode, kollektivName)}">${fCI_perf(stats.npv, 'npv')}</td><td>${stats.npv?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getStatTooltipHTML('acc', null, methode, kollektivName)}">Accuracy</td><td data-tippy-content="${_getStatTooltipHTML('acc', stats, methode, kollektivName)}">${fCI_perf(stats.acc, 'acc')}</td><td>${stats.acc?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getStatTooltipHTML('balAcc', null, methode, kollektivName)}">Balanced Accuracy</td><td data-tippy-content="${_getStatTooltipHTML('balAcc', stats, methode, kollektivName)}">${fCI_perf(stats.balAcc, 'balAcc')}</td><td>${stats.balAcc?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getStatTooltipHTML('f1', null, methode, kollektivName)}">F1-Score</td><td data-tippy-content="${_getStatTooltipHTML('f1', stats, methode, kollektivName)}">${fCI_perf(stats.f1, 'f1')}</td><td>${stats.f1?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getStatTooltipHTML('auc', null, methode, kollektivName)}">AUC (Bal. Acc.)</td><td data-tippy-content="${_getStatTooltipHTML('auc', stats, methode, kollektivName)}">${fCI_perf(stats.auc, 'auc')}</td><td>${stats.auc?.method || na}</td></tr>
        </tbody></table></div>`;
    ...
    function createVergleichContentHTML(stats, kollektivName) {
        ...
        let tableHTML = `<div class="table-responsive px-2"><table class="table table-sm table-striped small mb-0" id="table-vergleich-as-vs-t2-${kollektivName.replace(/\s+/g, '_')}"><thead><tr><th>Test</th><th>Statistik</th><th>p-Wert</th><th>Methode</th></tr></thead><tbody>
            <tr><td data-tippy-content="${_getTestTooltipHTML('mcnemar', null, kollektivName)}">McNemar (Accuracy)</td><td>${formatNumber(stats.mcnemar?.statistic, 3, na)} (df=${stats.mcnemar?.df || na})</td><td data-tippy-content="${_getTestTooltipHTML('mcnemar', stats, kollektivName)}">${fP(stats.mcnemar?.pValue)} ${getStatisticalSignificanceSymbol(stats.mcnemar?.pValue)}</td><td>${stats.mcnemar?.method || na}</td></tr>
            <tr><td data-tippy-content="${_getTestTooltipHTML('delong', null, kollektivName)}">DeLong (AUC)</td><td>Z=${formatNumber(stats.delong?.Z, 3, na)}</td><td data-tippy-content="${_getTestTooltipHTML('delong', stats, kollektivName)}">${fP(stats.delong?.pValue)} ${getStatisticalSignificanceSymbol(stats.delong?.pValue)}</td><td>${stats.delong?.method || na}</td></tr>
        </tbody></table></div>`;
        return tableHTML;
    }
    ...
    function createVergleichKollektiveContentHTML(stats, kollektiv1Name, kollektiv2Name) {
        ...
        const accASTooltip = _getTestTooltipHTML('accComp', accAS, `${kollektiv1Display} vs. ${kollektiv2Display}`, 'AS');
        const accT2Tooltip = _getTestTooltipHTML('accComp', accT2, `${kollektiv1Display} vs. ${kollektiv2Display}`, 'T2');
        const aucASTooltip = _getTestTooltipHTML('aucComp', aucAS, `${kollektiv1Display} vs. ${kollektiv2Display}`, 'AS');
        const aucT2Tooltip = _getTestTooltipHTML('aucComp', aucT2, `${kollektiv1Display} vs. ${kollektiv2Display}`, 'T2');
        ...
        tableHTML += `<tr><td>Accuracy</td><td>AS</td><td data-tippy-content="${accASTooltip}">${fP(accAS?.pValue)} ${getStatisticalSignificanceSymbol(accAS?.pValue)}</td><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.accComp?.description.replace('[METHODE]','AS')}">${accAS?.testName || na}</td></tr>`;
        tableHTML += `<tr><td>Accuracy</td><td>T2</td><td data-tippy-content="${accT2Tooltip}">${fP(accT2?.pValue)} ${getStatisticalSignificanceSymbol(accT2?.pValue)}</td><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.accComp?.description.replace('[METHODE]','T2')}">${accT2?.testName || na}</td></tr>`;
        tableHTML += `<tr><td>AUC</td><td>AS</td><td data-tippy-content="${aucASTooltip}">${fP(aucAS?.pValue)} ${getStatisticalSignificanceSymbol(aucAS?.pValue)} (Diff: ${formatNumber(aucAS?.diffAUC, 3, na)}, Z=${formatNumber(aucAS?.Z, 2, na)})</td><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.aucComp?.description.replace('[METHODE]','AS')}">${aucAS?.method || na}</td></tr>`;
        tableHTML += `<tr><td>AUC</td><td>T2</td><td data-tippy-content="${aucT2Tooltip}">${fP(aucT2?.pValue)} ${getStatisticalSignificanceSymbol(aucT2?.pValue)} (Diff: ${formatNumber(aucT2?.diffAUC, 3, na)}, Z=${formatNumber(aucT2?.Z, 2, na)})</td><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.aucComp?.description.replace('[METHODE]','T2')}">${aucT2?.method || na}</td></tr>`;
        tableHTML += `</tbody></table></div>`; return tableHTML;
    }
    ...
    function createCriteriaComparisonTableHTML(results, kollektivName) {
         ...
         const tc = TOOLTIP_CONTENT || {}; const cc = tc.criteriaComparison || {};
         const headers = [
    ...
    function _createPresentationView_ASPUR_HTML(presentationData) {
        ...
        const createPerfTableRow = (stats, kollektivKey) => { const kollektivDisplayName = getKollektivDisplayName(kollektivKey); const na = '--'; const fCI_p = (m, k) => { const d = (k === 'auc'||k==='f1') ? 3 : 1; const p = !(k === 'auc'||k==='f1'); return formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, d, p, na); }; const tooltipBaseKey = 'praesentation.asPurPerfTable'; const getTT = (mk) => { const base = TOOLTIP_CONTENT[tooltipBaseKey]?.[mk] || TOOLTIP_CONTENT.statMetrics[mk]?.description || mk; return base.replace('[METHODE]', 'AS').replace('[KOLLEKTIV]', kollektivDisplayName); }; const getInterpretationTT = (mk, st) => { const base = TOOLTIP_CONTENT.statMetrics[mk]?.interpretation || mk; return base.replace('[METHODE]', 'AS').replace('[KOLLEKTIV]', `<strong>${kollektivDisplayName}</strong>`).replace('[WERT]', `<strong>${fCI_p(st?.[mk], mk)}</strong>`).replace('[LOWER]', formatNumber(st?.[mk]?.ci?.lower, (mk==='auc'||mk==='f1')?3:1, na)).replace('[UPPER]', formatNumber(st?.[mk]?.ci?.upper, (mk==='auc'||mk==='f1')?3:1, na)).replace('[METHOD_CI]', st?.[mk]?.method || na).replace('[BEWERTUNG]', mk==='auc' ? getAUCBewertung(st?.auc?.value) : '').replace(/%/g,'%'); }; if (!stats || typeof stats.matrix !== 'object') return `<tr><td class="fw-bold" data-tippy-content="${getTT('kollektiv')}">${kollektivDisplayName} (N=?)</td><td colspan="6" class="text-muted text-center">Daten fehlen</td></tr>`; const count = stats.matrix ? (stats.matrix.rp + stats.matrix.fp + stats.matrix.fn + stats.matrix.rn) : 0; return `<tr><td class="fw-bold" data-tippy-content="${getTT('kollektiv')}">${kollektivDisplayName} (N=${count})</td><td data-tippy-content="${getInterpretationTT('sens', stats)}">${fCI_p(stats.sens, 'sens')}</td><td data-tippy-content="${getInterpretationTT('spez', stats)}">${fCI_p(stats.spez, 'spez')}</td><td data-tippy-content="${getInterpretationTT('ppv', stats)}">${fCI_p(stats.ppv, 'ppv')}</td><td data-tippy-content="${getInterpretationTT('npv', stats)}">${fCI_p(stats.npv, 'npv')}</td><td data-tippy-content="${getInterpretationTT('acc', stats)}">${fCI_p(stats.acc, 'acc')}</td><td data-tippy-content="${getInterpretationTT('auc', stats)}">${fCI_p(stats.auc, 'auc')}</td></tr>`; };
    ...
    function _createPresentationView_ASvsT2_HTML(presentationData, selectedStudyId = null, currentKollektiv = 'Gesamt') {
        ...
        if (canDisplayResults) {
            ...
            metrics.forEach(key => {
                 ...
                 const tooltipAS = _getStatTooltipHTML(key, statsAS, 'AS', kollektivName);
                 const tooltipT2 = _getStatTooltipHTML(key, statsT2, t2CriteriaLabelShort || 'T2', kollektivName);
                 comparisonTableHTML += `<tr><td>${metricNames[key]}</td><td data-tippy-content="${tooltipAS}">${valAS}</td><td data-tippy-content="${tooltipT2}">${valT2}</td></tr>`;
            });
            ...
            const mcNemarTooltip = _getTestTooltipHTML('mcnemar', vergleich, kollektivName, t2CriteriaLabelShort);
            const delongTooltip = _getTestTooltipHTML('delong', vergleich, kollektivName, t2CriteriaLabelShort);
            testsTableHTML += `<tr><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.mcnemar?.description}">McNemar (Acc)</td><td>${formatNumber(vergleich?.mcnemar?.statistic, 3, '--')} (df=${vergleich?.mcnemar?.df || '--'})</td><td data-tippy-content="${mcNemarTooltip}"> ${fPVal(vergleich?.mcnemar)} ${getStatisticalSignificanceSymbol(vergleich?.mcnemar?.pValue)}</td><td class="text-muted">${vergleich?.mcnemar?.method || '--'}</td></tr>`;
            testsTableHTML += `<tr><td data-tippy-content="${TOOLTIP_CONTENT.statMetrics.delong?.description}">DeLong (AUC)</td><td>Z=${formatNumber(vergleich?.delong?.Z, 3, '--')}</td><td data-tippy-content="${delongTooltip}"> ${fPVal(vergleich?.delong)} ${getStatisticalSignificanceSymbol(vergleich?.delong?.pValue)}</td><td class="text-muted">${vergleich?.delong?.method || '--'}</td></tr>`;
